...
### Added
- **`check skill:<name> --version <X.Y.Z>` and `check mcp:<name> --version <X.Y.Z>` render the rich-context block.** When the registry has a fresh `PackageNarrative` for the (type, name, version) tuple (POSTed by `secure --publish` in 0.20.0), `check` renders the v1 mockups from `briefs/check-rich-context-skills-mcp-v1.md` §3: hardcoded-secrets group with rotation guidance, declared-vs-observed permission delta (skill) or tool list + scope rows (MCP), severity-sorted findings, deterministic verdict reasoning, threat-model questions, action gradient with primary CTA. Rendering is byte-identical with `ai-trust check` and `opena2a check` against the same fixture (parity F12 / F13). Falls back to the legacy block + v1 footer when the registry has no narrative or when the `--version` flag is omitted (default-version lookup pending the Registry GET narrative endpoint extension — see follow-up brief).
