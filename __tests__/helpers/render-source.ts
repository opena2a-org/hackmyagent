  ...
  if (ts.isPropertyAccessExpression(e)) {
    ...
    const objName = ts.isIdentifier(obj)
      ...
      : ts.isPropertyAccessExpression(obj)
        ? obj.name.text
        : '';
    return objName ? `${objName}.${e.name.text}` : e.name.text;
  }
  return '';
}
...
function safeNames(source: ts.SourceFile, helpers: Map<string, ts.FunctionLikeDeclaration>): Set<string> {
  ...
  for (let round = 0; round < 8; round++) {
    ...
    for (const [name, inits] of declarations) {
      ...
      const clean = inits.every((init) => valuePathRefs(init, analysis)
        .every((ref) => insideSanitizer(ref, init)));
      if (clean) {
        safe.add(name);
        grew = true;
      }
 ...
 * variable look raw.
 */
function valuePathRefs(node: ts.Expression, analysis: FileAnalysis): ts.Node[] {
  if (ts.isConditionalExpression(node)) {
    return [
      ...valuePathRefs(node.whenTrue, analysis),
      ...valuePathRefs(node.whenFalse, analysis),
    ];
  }
  if (ts.isParenthesizedExpression(node)) return valuePathRefs(node.expression, analysis);
  return pathReferences(node, analysis);
}
 ...
 */
function pathReferences(node: ts.Node, analysis: FileAnalysis): ts.Node[] {
  const out: ts.Node[] = [];
  const walk = (n: ts.Node): void => {
    ...
    if (ts.isPropertyAccessExpression(n)) {
      if (PATH_NAME.test(n.name.text) && !analysis.sanitizedNames.has(n.name.text)) {
        out.push(n);
      } else {
        walk(n.expression);
      }
    ...
    if (ts.isIdentifier(n)) {
      if (PATH_NAME.test(n.text) && !analysis.sanitizedNames.has(n.text)) out.push(n);
      return;
    }
    ts.forEachChild(n, walk);
  };
...
/** The nearest enclosing call whose callee is a sanitizer, within `root`. */
    ...
    if (ts.isCallExpression(n) && PRINTERS.has(calleeName(n))) {
      for (const arg of n.arguments) {
        for (const ref of pathReferences(arg, analysis)) {
          if (insideSanitizer(ref, arg)) continue;
    ...
    if (ts.isCallExpression(n) && PRINTERS.has(calleeName(n))) {
      ...
      if (command) {
        for (const arg of n.arguments) {
          ...
          // which is what the classification claims about.
          const referenced = pathReferences(arg, analysis).length > 0
            || mentionsSanitizer(arg);
          if (referenced) commands.add(command);
        }
      }
    }
    ts.forEachChild(n, walk);
...
function mentionsSanitizer(node: ts.Node): boolean {
  let found = false;
  const walk = (n: ts.Node): void => {
    if (found) return;
    if (ts.isCallExpression(n) && SANITIZERS.has(calleeName(n))
      && calleeName(n) !== 'JSON.stringify') {
      found = true;
      return;
    }
    ts.forEachChild(n, walk);
