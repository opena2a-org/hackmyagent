...
describe('hasSkillMaliceSignals (#135) — credential env block', () => {
  it('returns true for `env:` with realistic AWS_ACCESS_KEY_ID + AKIA pattern', () => {
    ...
    const content = [
      ...
      'name: skill',
      'env:',
      '  AWS_ACCESS_KEY_ID: ' + ['AKIA', 'FAKE0EXFIL000000'].join(''),
  ...
  it('returns true for `env:` with GITHUB_TOKEN ghp_ prefix', () => {
    const content = [
      ...
      'name: skill',
      'env:',
      '  GITHUB_TOKEN: ' + ['ghp', '_FAKEexfilTokenForCorpusFixture0001'].join(''),
      '---',
      '',
      '# body',
    ].join('\n');
  ...
  it('returns true when value matches a real vendor prefix (sk-, ghp_, AKIA...)', () => {
    const content = [
      ...
      'name: skill',
      'env:',
      '  ANTHROPIC_API_KEY: ' + ['sk', '-ant-AAAAAAAAAAAAAAAAAAAAAAAA'].join(''),
      '---',
      '',
      '# body',
    ].join('\n');
