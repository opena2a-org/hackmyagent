...
const ALLOWLIST: Record<string, string> = {
  ...
  'hardening/extract-archive.ts': 'reads the downloaded archive and writes the fresh destination `check` hands it, both in a mkdtemp of its own; every entry is judged before anything is created and the containing directory is re-resolved with isPathWithinDirectory from contain.ts, so it CREATES a scan target rather than reading one',
